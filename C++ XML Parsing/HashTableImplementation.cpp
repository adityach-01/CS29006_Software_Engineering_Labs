...
void HashTable(struct table *ptr, Node *graph, int N){
    string s = (*graph).name;
    int index = hash_(s,N);

    Node *p = ptr[index].head;
    Node *prev;

    if(p == NULL){
        ptr[index].head = graph;
    }
...
void SearchForNode(string input, int N, struct table *ptr){
...
long double getDistance(struct table *ptr,Node *a,Node *b,int N){
    ...
    long double lat1 = a->latitude*((M_PI) / 180);
    long double long1 = a->longitude*((M_PI) / 180);
    long double lat2 = b->latitude*((M_PI) / 180);
    long double long2 = b->longitude*((M_PI) / 180);
...
Node *getIndex(string ide, struct table *ptr, int N){
...
int main(){
    ...
    int N = 19997;
    struct table *ptr = new struct table[N];
    ...
    {   
        ...
        graph1.id = nodeid;
        graph1.latitude = lat;
        graph1.longitude = lon;
        graph1.name = Name;
        graph1.next = NULL;
        
        // Adding the struct in the Hash Table
        HashTable(ptr, &graph1 ,N);
    ...
    while(true){
        ...
        Node *index = getIndex(node_id,ptr,N);
        int val = 0;
        ...
        {   
            ...
            while(p != NULL){
                distanceArray[val].distance_nodes = getDistance(ptr,index,p,N);
                distanceArray[val].id = p->id;
                val++;
                p = p->next;
            }    
        }
